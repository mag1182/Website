...
$landingExampleURL = "http://www.youtube.com/watch?v=Kob0G2hE8IY";
...
if (empty($url)) {
    if (empty($startURL)) {
      die("<html><head><title>Proxy</title></head><body><h1>Welcome to miniProxy!</h1>The Proxy can be directly invoked like this: <a href=\"" . $landingExampleURL . "\">" . $landingExampleURL . "</a><br /><br />You can simply watch any YouTube video by enter the URL below:<br /><br /><form onsubmit=\"if (document.getElementById('site').value) { window.location.href='" . "' + document.getElementById('site').value; return false; } else { window.location.href='" . $landingExampleURL . "'; return false; }\" autocomplete=\"off\"><input id=\"site\" type=\"text\" size=\"50\" /><input type=\"submit\" value=\"Proxy It!\" /></form></body></html>");
    } else {
      $url = $startURL;
    }
